    ...
    def loadImage(self, idx):
        ...
        for i in range(np.shape(orig_keypoints)[1]):
            if orig_keypoints[0, i, 0] > 0:
                ...
                orig_keypoints[0, i, 1] = int(orig_keypoints[0, i, 1] / orig_img.shape[0] * self.input_res)
...
def init(config):
    ...
    print("data train: {}, val: {}".format(len(train), len(valid)))
    ...
    def gen(phase):
        batchsize = config['train']['batchsize']
        iter_num = config['train']['{}_iters'.format(phase)]
